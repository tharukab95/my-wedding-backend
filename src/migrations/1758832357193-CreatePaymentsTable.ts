...
export class CreatePaymentsTable1758832357193 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "payments" ADD CONSTRAINT "FK_payments_adId" FOREIGN KEY ("adId") REFERENCES "matrimonial_ads"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "payments" ADD CONSTRAINT "FK_payments_pricingPlanId" FOREIGN KEY ("pricingPlanId") REFERENCES "pricing_plans"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX "IDX_payments_adId" ON "payments" ("adId")`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "payments" DROP CONSTRAINT "FK_payments_pricingPlanId"`,
    );
    await queryRunner.query(
      `ALTER TABLE "payments" DROP CONSTRAINT "FK_payments_adId"`,
    );
    await queryRunner.query(
